...
class mod_10 extends Module {
    ...
    public function load() {
        ...
        $qry = "SELECT id, name, description FROM dessi_investments WHERE step_id=".$this->get_step()." AND project_id=".$this->get_project_id()." ORDER BY priority";
        ...
        if ($is_invest) {
            ...
            $qry = 'SELECT mod3.id, mod3.num, mod3.category, mod3.type, mod3.question, mod3.help, mod3.show_if_x, mod3.and_x_contains, (CASE WHEN saved.id > 0 THEN 1 ELSE 0 END) as exist, saved.id as saved_id, saved.comment FROM mod3 LEFT JOIN mod3_answers_save saved ON saved.question_id=mod3.id AND saved.project_id='.$this->get_project_id().' AND saved.investment_id ='.$safe["var1"].' WHERE mod3.step_id='.$this->get_step().' ORDER BY mod3.num';
            $res = $this->db->query($qry);
            if($res && $res->num_rows) {
...
$(document).ready(function() {
    ...
    private function ins_mod3_answers_save($question_id, $comment_str) {
        ...
        $qry = sprintf("INSERT INTO mod3_answers_save (question_id, project_id, step_id, investment_id, comment) VALUES ('%d','%d','%d','%d','%s');",
                       (int) $question_id,
    ...
    private function upd_mod3_answers_save($question_id, $comment_str) {
        ...
        $qry = sprintf("UPDATE mod3_answers_save SET comment='%s' WHERE question_id=%d AND project_id=%d AND step_id=%d AND investment_id=%d;",
                       $this->db->real_escape_string($comment_str),
                       (int) $question_id,
                       (int) $this->get_project_id(),
                       (int) $this->get_step(),
    ...
    public function save() {
        ...
        if(isset($_POST["invest_name"]) && !empty($_POST["invest_name"])) {
            ...
            $qry = "SELECT COUNT(id) as amount FROM dessi_investments WHERE step_id=".$this->get_step()." AND project_id=".$this->get_project_id().";";
            $res = $this->db->query($qry);
            $priority = 1;
            if($res && $res->num_rows) {
            ...
            $qry = sprintf('INSERT INTO dessi_investments (project_id, step_id, enabled, priority, name, description) VALUES ("%d", "%d", "%d", "%d", "%s", "%s");',
                           $this->get_project_id(),
                           $this->get_step(),
                           1,
                           $priority,
            ...
            $qry = "SELECT mod3.id, mod3.type, mod3.show_if_x, mod3.and_x_contains as contains FROM mod3 WHERE mod3.step_id=".$this->get_step()." ORDER BY mod3.num";
            $res = $this->db->query($qry);
            $checklist = array();
            if($res && $res->num_rows) {
            ...
            $qry = "SELECT mod3.id, mod3.type, (saved.id > 0) as exist, saved.id as saved_id FROM mod3 LEFT JOIN mod3_answers_save saved ON saved.question_id=mod3.id AND saved.project_id=".$this->get_project_id()." AND saved.investment_id=".$this->get_var1()." WHERE mod3.step_id=".$this->get_step()." ORDER BY mod3.num";
            ...
            if($res && $res->num_rows) {
                while($posts = $res->fetch_object()) {
                    ...
                    if ((empty($comment_str) && !$isvalue) || !in_array($posts->id, $checklist)) {
                        ...
                        if ($posts->exist) {
                            $qry = "DELETE FROM mod3_answers_save WHERE question_id = ".$posts->id." AND project_id = ".$this->get_project_id()." AND step_id = ".$this->get_step().";";
    ...
    public function remove_investment(){
        ...
        $qry = 'SELECT saved.id saved_id, type FROM mod3 LEFT JOIN mod3_answers_save saved ON saved.question_id = mod3.id AND saved.project_id='.$this->get_project_id().' AND saved.step_id = '.$this->get_step().' WHERE saved.investment_id = '.$this->get_var1().';';
        ...
        $res = $this->db->query('DELETE FROM mod3_answers_save WHERE project_id = '.$this->get_project_id().' AND step_id = '.$this->get_step());
        $res = $this->db->query("DELETE FROM dessi_investments WHERE project_id=".$this->get_project_id()." AND id=".$this->get_var1()." AND step_id = ".$this->get_step().";");
    }

    public function reset_step() {
        $qry = 'SELECT saved.id saved_id, type FROM mod3 LEFT JOIN mod3_answers_save saved ON saved.question_id = mod3.id AND saved.project_id='.$this->get_project_id().' AND saved.step_id = '.$this->get_step().';';
        ...
        $res = $this->db->query('DELETE FROM mod3_answers_str WHERE answer_id in ('.implode(",", $rem_obj["str"]).')');
        $res = $this->db->query('DELETE FROM mod3_answers_save WHERE project_id = '.$this->get_project_id().' AND step_id = '.$this->get_step());
        $res = $this->db->query("DELETE FROM dessi_investments WHERE project_id=".$this->get_project_id()." AND step_id = ".$this->get_step().";");
    ...
    public function report() {
        ...
        if($res && $res->num_rows) {
            ...
            /* Get data, and existing */
                $qry = 'SELECT mod3.id, mod3.num, mod3.category, mod3.type, mod3.question, mod3.help, mod3.show_if_x, mod3.and_x_contains, (CASE WHEN saved.id > 0 THEN 1 ELSE 0 END) as exist, saved.id as saved_id, saved.comment FROM mod3 LEFT JOIN mod3_answers_save saved ON saved.question_id=mod3.id AND saved.project_id='.$this->get_project_id().' AND saved.investment_id ='.$invest_res->id.' WHERE mod3.step_id='.$this->get_step().' AND saved.id > 0 ORDER BY mod3.num';
